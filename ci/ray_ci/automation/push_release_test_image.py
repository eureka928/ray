...
def _run_gcloud_docker_login() -> str:
    """Authenticate with GCP Artifact Registry using gcloud.

    The gcloud script downloads and installs the gcloud SDK, then configures
    Docker to use docker-credential-gcloud for GCP registries. After running,
    we add the gcloud SDK to PATH so crane can find the credential helper.

    Returns:
        Path to the temporary install directory (caller should clean up).
    """
    script_path = _runfiles.Rlocation("io_ray/release/gcloud_docker_login.sh")
    credentials_path = _runfiles.Rlocation("io_ray/release/aws2gce_iam.json")

    # Use system Python for gcloud SDK installer (bazel-managed Python lacks
    # the bootstrapping module that gcloud expects)
    env = os.environ.copy()
    env["CLOUDSDK_PYTHON"] = "/usr/bin/python3"

    # Install to a temp directory so we can clean up after
    gcloud_install_dir = tempfile.mkdtemp(prefix="gcloud-install-")

    logger.info("Authenticating with GCP Artifact Registry...")
    result = subprocess.run(
        ["bash", script_path, credentials_path],
        capture_output=True,
        text=True,
        env=env,
        cwd=gcloud_install_dir,
    )
    if result.returncode != 0:
        shutil.rmtree(gcloud_install_dir, ignore_errors=True)
        raise PushReleaseTestImageError(f"GCP authentication failed: {result.stderr}")

    # Add gcloud SDK to PATH so crane can find docker-credential-gcloud
    gcloud_bin = os.path.join(gcloud_install_dir, "google-cloud-sdk", "bin")
    os.environ["PATH"] = f"{gcloud_bin}:{os.environ.get('PATH', '')}"

    return gcloud_install_dir
...
def main(
    ...
    gcloud_install_dir: Optional[str] = None
    try:
        gcloud_install_dir = _run_gcloud_docker_login()
